...
bool GetServer(char* call, short port, sockaddr* from, int* flen)
{
	...
	if (setsockopt(clientSocket,
		SOL_SOCKET,														// уровень действия режима
		SO_BROADCAST,													// режим сокета для использования broadcast
	...
	{
		cout << endl << "Сервер с именем " << call << " найден.";
	}
	else
	{
		cout << endl << "Сервер с именем " << call << " не найден.";
		return false;
	}
